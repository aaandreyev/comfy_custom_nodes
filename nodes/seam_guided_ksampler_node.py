...
class SeamGuidedKSamplerNode:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "anchor_width_latent_px": ("INT", {"default": 3, "min": 1, "max": 256, "step": 1,
                                          "tooltip": "Latent-grid cells, not image pixels (flux2: 1 cell ≈ 16 image px)."}),
                "anchor_falloff_latent_px": ("INT", {"default": 16, "min": 1, "max": 512, "step": 1,
                                            "tooltip": "Latent-grid cells, not image pixels (flux2: 1 cell ≈ 16 image px)."}),
            ...
            "optional": {
                ...
                "safety_ring_latent_px": ("INT", {"default": 16, "min": 0, "max": 512, "step": 1,
                                         "tooltip": "Latent-grid cells, not image pixels (flux2: 1 cell ≈ 16 image px)."}),
                "low_freq_anchor_strength": ("FLOAT", {"default": 0.0, "min": 0.0, "max": 2.0, "step": 0.05}),
                "low_freq_anchor_decay_latent_px": ("INT", {"default": 64, "min": 1, "max": 1024, "step": 1,
                                                   "tooltip": "Latent-grid cells, not image pixels (flux2: 1 cell ≈ 16 image px)."}),
            },
        }
    ...
    def sample(
        ...
        anchor_width_latent_px,
        anchor_falloff_latent_px,
        process_left,
        process_right,
        process_top,
        process_bottom,
        ...
        safety_ring_latent_px=16,
        low_freq_anchor_strength=0.0,
        low_freq_anchor_decay_latent_px=64,
    ):
        ...
        anchor_state = prepare_seam_anchor_state(
            ...
            anchor_width_px=int(anchor_width_latent_px),
            anchor_falloff_px=int(anchor_falloff_latent_px),
            ...
            safety_ring_px=int(safety_ring_latent_px),
            low_freq_anchor_decay_px=int(low_freq_anchor_decay_latent_px),
        )
