...
from ..runtime.infer.seam_profile_tone_match import apply_seam_profile_tone_match


class SeamProfileToneMatchNode:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "image": ("IMAGE",),
                "mask": ("MASK", {"tooltip": "Stitch mask: the region that will be composited over the reference. "
                                  "If you feed a grown mask, set seam_inset_px to the growth amount."}),
                "inner_width": ("INT", {"default": 64, "min": 1, "max": 1024, "step": 1,
                ...
                "seam_inset_px": ("INT", {"default": 0, "min": 0, "max": 256, "step": 1,
                                  "tooltip": "The stitch seam lies this many px inside the supplied mask "
                                             "(e.g. 15 when the mask went through GrowMask expand=15)."}),
    ...
    def run(
        ...
        if (mask_t > 0.5).sum() == 0:
            raise ValueError(
                "SeamProfileToneMatchNode: mask has no active pixels (no values above 0.5). "
                "Check that the mask is correctly connected and not empty."
            )

        corrected_rgb, debug = apply_seam_profile_tone_match(
            ref_rgb,
            img_rgb,
            mask_t,
